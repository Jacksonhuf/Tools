...
| `FUNCTION_INVOCATION_FAILED` on `/api/*` | Redeploy after `npm run build:vercel` (bundles `api/index.js`). Ensure production env vars from `config/vercel.env.production.example` are set. |
