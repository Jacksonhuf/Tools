...
export interface DebounceRecordInput {
  listing_id: string;
  channel: SalesChannel;
  offer_id: string;
  previous_effective: number | null;
...
export interface DebounceBackend {
  ...
  record(input: DebounceRecordInput): Promise<void>;
  flush(listingId: string): Promise<CompetitorPriceChangedPayload | null>;
...
export class MemoryDebounceBackend implements DebounceBackend {
  ...
  async flush(listingId: string): Promise<CompetitorPriceChangedPayload | null> {
...
export function resetMemoryDebounceWindows(): void {
  windows.clear();
}
